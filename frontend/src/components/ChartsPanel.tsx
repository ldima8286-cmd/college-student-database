...
export default function ChartsPanel({ stats }: Props) {
  ...
  const PerfChart = () => (
    <div className="card">
      <h4 className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-4">
        <TrendingUp className="w-4 h-4 text-primary-600" /> Средняя успеваемость по курсам
      </h4>
      <div className="flex items-end gap-3 h-40">
        {courses.map((c) => (
          <div key={c.course} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
            ...
            />
            <span className="text-xs text-gray-500 dark:text-gray-400">{c.course}к</span>
          </div>
        ))}
      </div>
    </div>
  ...
  const AttChart = () => (
    <div className="card">
      <h4 className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-4">
        <Activity className="w-4 h-4 text-emerald-600" /> Средняя посещаемость по курсам
      </h4>
      <div className="flex items-end gap-3 h-40">
        {courses.map((c) => (
          <div key={c.course} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
  ...
  const DebtChart = () => {
    ...
    return (
      <div className="card">
        <h4 className="flex items-center gap-2 text-sm font-semibold text-gray-600 dark:text-gray-300 mb-4">
          <BarChart3 className="w-4 h-4 text-amber-600" /> Количество студентов по курсам
        </h4>
        <div className="flex items-end gap-3 h-40">
          {courses.map((c) => (
            <div key={c.course} className="flex-1 flex flex-col items-center gap-1 h-full justify-end">
