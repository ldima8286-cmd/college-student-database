...
Веб-приложение для учёта учащихся колледжа: ведение карточек студентов, управление академическими задолженностями, поиск и фильтрация, импорт/экспорт данных, авторизация с ролями, аудит действий администраторов, тёмная тема и установка как PWA-приложение. Дополнительно: электронный дневник студента (оценки по предметам), недельное расписание занятий по группам и роль куратора группы.
...
| **Авторизация** | Регистрация, вход по email/паролю, JWT access/refresh-токены, bcrypt-хеширование паролей, роли `admin`, `curator` и `user` |
| **Регистрация** | При регистрации указывается группа и курс — автоматически создаётся карточка студента, которая сразу видна администратору и куратору группы |
| **Куратор** | Роль `curator` привязана к группе: полный доступ к данным студентов своей группы, выставление оценок, просмотр расписания |
| **Электронный дневник** | Студент видит свою анкету, оценки по предметам и расписание группы во вкладках |
| **Расписание** | Недельное расписание занятий по группам, редактируется администратором на отдельной странице |
| **Предметы** | Справочник учебных дисциплин (используется в оценках и расписании) |
| **Оценки** | Выставление оценок по предметам преподавателями/кураторами, студент видит только свои; 10-балльная шкала (как в РБ: 1–10) |
| **Телефон** | Автоматическое форматирование номера телефона (до 15 цифр) при вводе в формах и в карточках |
...
| Роль | Email | Пароль | Группа |
|------|-------|--------|--------|
| Администратор | `admin@college.local` | `admin123` | — |
| Куратор | `curator@college.local` | `curator123` | `ПО-507` |
...
| `CURATOR_EMAIL` | `curator@college.local` | Email куратора (создаётся при первом запуске) |
| `CURATOR_PASSWORD` | `curator123` | Пароль куратора |
| `CURATOR_GROUP` | `ПО-507` | Группа куратора |
...
| `AUDIT_RETENTION_DAYS` | `30` | Хранить записи аудит-лога не дольше N дней |
| `AUDIT_MAX_ROWS` | `2000` | Максимальное число записей аудит-лога (старые удаляются) |
...
| `GET` | `/api/groups` | Список групп студентов (для ролей `admin` и `curator`) |
| `GET/POST` | `/api/subjects` | Список / создание предметов |
| `DELETE` | `/api/subjects/:id` | Удаление предмета (роль `admin`) |
| `GET/POST` | `/api/schedule` | Расписание группы / создание занятия |
...
| `PUT` | `/api/admin/users/:id` | Изменение роли/группы пользователя (роль `admin`) |
...
│   │   ├── auth.ts             # JWT, регистрация, вход, роли (admin/curator/user)
│   │   ├── schema.ts           # Схема БД (PostgreSQL, Drizzle) — users, students, subjects, schedule, marks
...
│   │   ├── utils/phone.ts      # Санитизация и форматирование телефона
│   │   ├── pages/              # Login, Register, Dashboard, StudentDiary, CuratorPanel, AdminPanel, AuditLog, Profile
│   │   ├── pages/admin/        # SubjectsManager, ScheduleManager, UsersManager
│   │   ├── components/         # StudentTable, StudentForm, MarksView, MarksEditor, ScheduleView, StatsPanel и др.
